...
const LEVEL_GATED_PATHS: { path: string; minLevel: number }[] = [
  { path: "/console",    minLevel: 3 },
  { path: "/missions",   minLevel: 2 }, // [U-002] LV2以上のみアクセス可
  { path: "/classified", minLevel: 5 }, // [U-002] LV5以上のみアクセス可（最高機密）
];
...
export async function middleware(req: NextRequest) {
  ...
  if (PUBLIC_PATHS.some((p) => pathname.startsWith(p))) {
    // ログイン済みユーザーが /login にアクセスした場合はダッシュボードへリダイレクト
  ...
  // ── レベルゲート ──────────────────────────────────────────────
  for (const gate of LEVEL_GATED_PATHS) {
    if (pathname.startsWith(gate.path)) {
      const userLevel = Math.floor(session.level ?? 0);
      if (userLevel < gate.minLevel) {
        return NextResponse.redirect(new URL("/dashboard", req.url));
      }
    }
  }
