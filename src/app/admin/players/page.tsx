...
export default function PlayersPage() {
  ...
  const savePlayer = async () => {
    ...
    try {
      await apiFetch(`/api/admin/users/${selected.id}`, {
        ...
        body: JSON.stringify({ role: editRole, status: editStatus, displayName: editName, anomalyScore: editAnomaly, xpTotal: editXP }),
      });
      showToast("保存しました");
      await loadPlayers();
  ...
  const setXp = async () => {
    ...
    try {
      await apiFetch(`/api/admin/users/${selected.id}`, {
        method: "PUT",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({ xpTotal: editXP }),
      });
      showToast(`XPを${editXP}に設定しました`);
      await selectPlayer(selected.id);
    } catch { showToast("XP設定失敗", "err"); }
    finally { setSaving(false); }
  };
  ...
  const deletePlayer = async () => {
    ...
    try {
      await apiFetch(`/api/admin/users/${selected.id}`, { method: "DELETE" });
                ...
                <div style={{ display: "flex", flexDirection: "column", gap: 14 }}>
                  ...
                  <FieldGroup label="XP直接設定">
                    <div style={{ display: "flex", gap: 8, alignItems: "center" }}>
                      ...
                      <button onClick={setXp} disabled={saving} style={{ background: "none", border: `1px solid ${S.green}`, color: S.green, fontFamily: S.mono, fontSize: 10, padding: "7px 14px", cursor: "pointer", opacity: saving ? 0.5 : 1 }}>{saving ? "..." : "SET"}</button>
                    </div>
                  </FieldGroup>
                ...
                <div style={{ display: "flex", flexDirection: "column", gap: 14 }}>
                  ...
                  <button
                    onClick={async () => {
                      ...
                      try {
                        const res = await apiFetch(`/api/admin/users/${selected.id}/reset-password`, {
                          method: "POST",
                          headers: { "Content-Type": "application/json" },
