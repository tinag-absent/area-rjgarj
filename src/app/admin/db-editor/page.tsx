...
const QUICK_QUERIES: { label: string; sql: string }[] = [
  ...
  { label: "最近のチャット", sql: "SELECT cm.id, cm.chat_id, cm.sender_name, cm.text, cm.created_at FROM chat_messages cm ORDER BY cm.created_at DESC LIMIT 50;" },
  { label: "XPランキング", sql: "SELECT username, COALESCE(xp_total, 0) AS xp, clearance_level FROM users WHERE deleted_at IS NULL ORDER BY xp DESC LIMIT 20;" },
...
export default function DbEditorPage() {
  ...
  const loadTable = useCallback(async (name: string) => {
    ...
    try {
      const res = await apiFetch(`/api/admin/db-query?table=${encodeURIComponent(name)}`);
