...
export default function NpcScriptsPage() {
  ...
  const loadScripts = useCallback(async () => {
    ...
    try { const r = await apiFetch("/api/admin/npc-script"); const d = await r.json(); setScripts(Array.isArray(d) ? d : []); }
  ...
  async function loadRules(type: "trigger" | "idle" | "reaction" | "schedule" | "broadcast" | "condition") {
    if (type === "trigger") { setTriggersLoading(true); try { const r = await apiFetch("/api/admin/npc-engine-rules?type=trigger"); const d = await r.json(); setTriggers(Array.isArray(d) ? d : []); } finally { setTriggersLoading(false); } }
    if (type === "idle") { setIdlesLoading(true); try { const r = await apiFetch("/api/admin/npc-engine-rules?type=idle"); const d = await r.json(); setIdles(Array.isArray(d) ? d : []); } finally { setIdlesLoading(false); } }
    if (type === "reaction") { setReactionsLoading(true); try { const r = await apiFetch("/api/admin/npc-engine-rules?type=reaction"); const d = await r.json(); setReactions(Array.isArray(d) ? d : []); } finally { setReactionsLoading(false); } }
    if (type === "schedule") { setSchedulesLoading(true); try { const r = await apiFetch("/api/admin/npc-engine-rules?type=schedule"); const d = await r.json(); setSchedules(Array.isArray(d) ? d : []); } finally { setSchedulesLoading(false); } }
    if (type === "broadcast") { setBroadcastsLoading(true); try { const r = await apiFetch("/api/admin/npc-engine-rules?type=broadcast"); const d = await r.json(); setBroadcasts(Array.isArray(d) ? d.map((x: BroadcastEvent) => ({ ...x, sequence: x.sequence || [] })) : []); } finally { setBroadcastsLoading(false); } }
    if (type === "condition") { setConditionsLoading(true); try { const r = await apiFetch("/api/admin/npc-engine-rules?type=condition"); const d = await r.json(); setConditions(Array.isArray(d) ? d : []); } finally { setConditionsLoading(false); } }
  }
