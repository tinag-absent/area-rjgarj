...
// ─── ユーティリティ ──────────────────────────────────────────────────────────
// ビルド後はモジュールが再ロードされないため、プロセス内メモリキャッシュが有効
...
export default async function DivisionDetailPage({ params }: { params: Promise<{ slug: string }> }) {
  ...
  // 4つの静的JSONを並列で読み込む（直列→並列化）
  const [missionData, personnelData, incidentData, divisionData] = await Promise.all([
    Promise.resolve(loadJson<{ missions: Mission[] }>("mission-data.json")),
    Promise.resolve(loadJson<{ personnel: Personnel[] }>("personnel-data.json")),
    Promise.resolve(loadJson<{ incidents: Incident[] }>("area-incidents-data.json")),
    Promise.resolve(loadJson<{ divisions: DivisionData[] }>("divisions-data.json")),
  ]);
  const { missions } = missionData;
  const { personnel } = personnelData;
  const { incidents } = incidentData;
  const { divisions: divData } = divisionData;
