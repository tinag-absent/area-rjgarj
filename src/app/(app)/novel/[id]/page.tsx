...
type CardData =
  ...
  | { type: "LOCATION";  id: string; name: string; locationType?: string; description?: string; coordinates?: string; securityLevel?: number; facilities?: string[]; }
...
const ALERT_LEVEL_CONFIG: Record<string, { color: string; bg: string; border: string; label: string; icon: string }> = {
  critical: { color: "#ff2222", bg: "rgba(200,0,0,0.12)",   border: "rgba(255,34,34,0.6)",  label: "緊急警報",   icon: "warning" },
  high:     { color: "#f97316", bg: "rgba(249,115,22,0.10)", border: "rgba(249,115,22,0.55)", label: "高度警戒",   icon: "triangle" },
  medium:   { color: "#eab308", bg: "rgba(234,179,8,0.10)",  border: "rgba(234,179,8,0.5)",  label: "警戒",       icon: "diamondfill" },
  info:     { color: "#00c8ff", bg: "rgba(0,200,255,0.08)",  border: "rgba(0,200,255,0.4)",  label: "通達",       icon: "diamond" },
};
        ...
        <div style={{
          ...
          <div style={{ display: "flex", alignItems: "center", gap: "0.75rem", marginBottom: "0.6rem" }}>
            ...
            <div style={{
              ...
              <NovelSvgIcon icon={cfg.icon} color={cfg.color} />
            </div>
            <div>
...
const NPC_CHAT_COLORS: Record<string, { name: string; bg: string; border: string; icon: string }> = {
  "K-ECHO": { name: "#00c8ff", bg: "rgba(0,200,255,0.07)",   border: "rgba(0,200,255,0.3)",   icon: "diamond" },
  "N-VEIL": { name: "#a064ff", bg: "rgba(160,100,255,0.07)", border: "rgba(160,100,255,0.3)", icon: "target" },
  "L-RIFT": { name: "#50dc78", bg: "rgba(80,220,120,0.07)",  border: "rgba(80,220,120,0.3)",  icon: "hexagon" },
  "A-PHOS": { name: "#ffb43c", bg: "rgba(255,180,60,0.07)",  border: "rgba(255,180,60,0.3)",  icon: "heart" },
  "G-MIST": { name: "#a0a0a0", bg: "rgba(160,160,160,0.06)", border: "rgba(160,160,160,0.25)",icon: "wave" },
  "PLAYER": { name: "var(--primary)", bg: "rgba(0,255,255,0.07)", border: "rgba(0,255,255,0.25)", icon: "circletarget" },
};
function getNpcStyle(speaker: string) {
  return NPC_CHAT_COLORS[speaker] ?? { name: "#c084fc", bg: "rgba(168,85,247,0.07)", border: "rgba(168,85,247,0.25)", icon: "diamondfill" };
}
...
const CONSOLE_TYPE_CONFIG: Record<string, { color: string; icon: string; bg: string }> = {
  alert:      { color: "#ef4444", icon: "warning",    bg: "rgba(239,68,68,0.08)" },
  critical:   { color: "#ff2222", icon: "skull",      bg: "rgba(255,0,0,0.1)" },
  deployment: { color: "#a064ff", icon: "play",       bg: "rgba(160,100,255,0.08)" },
  action:     { color: "#00c8ff", icon: "diamond",    bg: "rgba(0,200,255,0.06)" },
  discovery:  { color: "#fbbf24", icon: "target",     bg: "rgba(245,158,11,0.07)" },
  completed:  { color: "#50dc78", icon: "check",      bg: "rgba(80,220,120,0.07)" },
};
          ...
          {entries.slice(0, visible).map((entry, i) => {
            ...
            return (
              <div key={i} style={{
                ...
                <span style={{ color: cfg.color, flexShrink: 0, fontSize: "0.7rem", marginTop: "1px" }}><NovelSvgIcon icon={cfg.icon} color={cfg.color} /></span>
                <div style={{ flex: 1 }}>
                  {entry.timestamp && (
...
function DataCard({ data, onClose, anchorRect }: {
  ...
  if (data.type === "ENTITY")    { headerColor = "#00c8ff"; headerBg = "rgba(0,200,255,0.07)";  typeLabel = "実体記録";     titleText = `${data.code ?? ""} ${data.name}`.trim(); }
  if (data.type === "LOCATION")  { headerColor = "#fbbf24"; headerBg = "rgba(245,158,11,0.07)"; typeLabel = "拠点情報";     titleText = data.name; }
  if (data.type === "MODULE")    { headerColor = "#50dc78"; headerBg = "rgba(80,220,120,0.07)"; typeLabel = "装備記録";     titleText = `${data.code ?? ""} ${data.name}`.trim(); }
  ...
  return (
    <div ref={cardRef} style={{
      ...
      <div style={{ padding: "0.75rem 1rem 1rem", overflowY: "auto", maxHeight: "380px" }}>
        ...
        {data.type === "ENTITY" && <>
          <div style={{ marginBottom: "0.6rem" }}>
            {data.classification && badge(data.classification.toUpperCase(), clsColor(data.classification))}
        ...
        {data.type === "LOCATION" && <>
          {data.locationType && <>{label("施設種別")}{val(data.locationType)}</>}
...
function NovelSvgIcon({ icon, color, size = 14 }: { icon: string; color: string; size?: number }) {
  const p = { width:size, height:size, fill:"none" as const, stroke:color, strokeWidth:"1.8" as const, strokeLinecap:"round" as const, strokeLinejoin:"round" as const };
  ...
  if (icon === "triangle")     return <svg {...p} viewBox="0 0 24 24"><polygon points="12 2 22 22 2 22" /></svg>;
  if (icon === "diamondfill")  return <svg {...p} fill={color} viewBox="0 0 24 24"><polygon points="12 2 22 12 12 22 2 12" /></svg>;
  ...
  if (icon === "heart")        return <svg {...p} viewBox="0 0 24 24"><path d="M20.84 4.61a5.5 5.5 0 0 0-7.78 0L12 5.67l-1.06-1.06a5.5 5.5 0 0 0-7.78 7.78l1.06 1.06L12 21.23l7.78-7.78 1.06-1.06a5.5 5.5 0 0 0 0-7.78z" /></svg>;
  if (icon === "wave")         return <svg {...p} viewBox="0 0 24 24"><path d="M2 12c1.5-3 3-4.5 4.5-4.5s3 3 4.5 3 3-4.5 4.5-4.5S18 9 19.5 9 21 12 22 12" /></svg>;
  ...
  if (icon === "skull")        return <svg {...p} viewBox="0 0 24 24"><circle cx="9" cy="10" r="1" /><circle cx="15" cy="10" r="1" /><path d="M8 16v2" /><path d="M12 16v2" /><path d="M16 16v2" /><path d="M4 15.5A8 8 0 0 1 12 4a8 8 0 0 1 8 11.5c0 .83-.67 1.5-1.5 1.5h-13A1.5 1.5 0 0 1 4 15.5z" /></svg>;
  if (icon === "play")         return <svg {...p} viewBox="0 0 24 24"><polygon points="5 3 19 12 5 21 5 3" /></svg>;
  if (icon === "check")        return <svg {...p} strokeWidth="2.2" viewBox="0 0 24 24"><polyline points="20 6 9 17 4 12" /></svg>;
