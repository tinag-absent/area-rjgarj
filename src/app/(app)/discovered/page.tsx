...
const CATEGORIES = [
  { id: "all",       label: "すべて",       color: "#00ffff", icon: "diamond",     desc: "全データ" },
  { id: "mission",   label: "収束案件",     color: "#ef4444", icon: "bolt",        desc: "作戦記録" },
  { id: "entity",    label: "海蝕実体",     color: "#a855f7", icon: "target",      desc: "実体データ" },
  { id: "module",    label: "モジュール",   color: "#eab308", icon: "hexagon",     desc: "装備・機器" },
  { id: "location",  label: "ロケーション", color: "#10b981", icon: "location",    desc: "拠点・施設" },
  { id: "personnel", label: "人員",         color: "#f97316", icon: "diamond",     desc: "機関員記録" },
  { id: "novel",     label: "ノベル",       color: "#06b6d4", icon: "diamondfill", desc: "記録文書" },
  { id: "post",      label: "掲示板",       color: "#64748b", icon: "chevron",     desc: "投稿記録" },
] as const;
    ...
    >
      ...
      >
        ...
        <div style={{
          ...
          <DiscovIcon icon={cat.icon} color={cat.color} />
        </div>
...
function DiscovIcon({ icon, color }: { icon: string; color: string }) {
...
export default function DiscoveredPage() {
  const items              = useDiscoveredStore((s) => s.items);
  const removeItem         = useDiscoveredStore((s) => s.removeItem);
  const clearAll           = useDiscoveredStore((s) => s.clearDiscovered);
  const clearByCategory    = useDiscoveredStore((s) => s.clearDiscoveredByCategory);
      ...
      <div style={{
        ...
        {CATEGORIES.map(cat => {
          ...
          return (
            ...
            >
              <span style={{ opacity: isActive ? 1 : hasData ? 0.7 : 0.3 }}><DiscovIcon icon={cat.icon} color={cat.color} /></span>
              <span>{cat.label}</span>
              <span style={{
