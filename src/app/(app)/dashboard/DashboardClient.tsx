...
// [FIX NEW-003] ハードコーディングを廃止してconstantsの値を使用
const LEVEL_THRESHOLDS_ARR = LEVEL_THRESHOLDS;
...
function DashNavIcon({ icon }: { icon: string }) {
  const p = { width:16, height:16, fill:"none" as const, stroke:"currentColor", strokeWidth:"1.8" as const, strokeLinecap:"round" as const, strokeLinejoin:"round" as const };
  ...
  if (icon === "bulletin")  return <svg {...p} viewBox="0 0 24 24"><rect x="3" y="3" width="18" height="18" rx="2" /><line x1="3" y1="9" x2="21" y2="9" /><line x1="3" y1="15" x2="21" y2="15" /><line x1="9" y1="9" x2="9" y2="21" /></svg>;
  if (icon === "skilltree") return <svg {...p} viewBox="0 0 24 24"><circle cx="12" cy="4" r="2" /><circle cx="5" cy="20" r="2" /><circle cx="19" cy="20" r="2" /><line x1="12" y1="6" x2="12" y2="13" /><line x1="12" y1="13" x2="5" y2="18" /><line x1="12" y1="13" x2="19" y2="18" /></svg>;
  if (icon === "triangle")  return <svg {...p} viewBox="0 0 24 24"><polygon points="12 2 22 22 2 22" /></svg>;
  if (icon === "classified") return <svg {...p} viewBox="0 0 24 24"><rect x="3" y="11" width="18" height="11" rx="2" ry="2" /><path d="M7 11V7a5 5 0 0 1 10 0v4" /></svg>;
...
export default function DashboardClient({
  ...
  // ⑥ 秘密の質問未設定警告（props から初期値を設定し、useEffect のAPI結果で確定）
  const [showSecretWarning, setShowSecretWarning] = useState(!user.hasSecretQuestion);
  ...
  // [K-003] level 5 はMAXなので nextLevelXp を level+1 から取得しない（分母0防止）
  const nextLevelXp = level >= 5 ? currentLevelXp + 1 : (LEVEL_THRESHOLDS_ARR[level + 1] ?? currentLevelXp + 1);
  ...
  useEffect(() => {
    async function checkTriggers() {
      try {
        const res = await apiFetch("/api/users/me/check-triggers", { method: "POST" });
        const data = await res.json();
        if (data.fired && data.fired.length > 0) {
          // 新しい通知を取得
          const notifRes = await apiFetch("/api/users/me/notifications");
          if (notifRes.ok) {
  ...
  // ⑥ 秘密の質問未設定チェック（K-002: APIで最新状態を取得し、設定済みなら警告を消す）
  useEffect(() => {
    apiFetch("/api/users/me/secret-question")
      .then(r => r.json())
      .then(d => { setShowSecretWarning(d.question === null || d.question === undefined); })
      .catch(() => {});
  }, []);

  async function handleDailyLogin() {
    if (dailyClaimed) return;
    // [FIX B4] 生fetch → apiFetch でCSRFヘッダーを付与
    const res = await apiFetch("/api/users/me/daily-login", { method: "POST" });
    const data = await res.json();
    if (data.success) {
  ...
  return (
        ...
        <div className="card">
          ...
          <div className="card-content">
            <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: "0.5rem" }}>
              {[
                { href: "/chat", label: "通信ログ", icon: "target", level: 1 },
                { href: "/reports", label: "レポート", icon: "grid", level: 1 },
                { href: "/bulletin", label: "掲示板", icon: "bulletin", level: 1 },
                { href: "/skill-tree", label: "スキルツリー", icon: "skilltree", level: 1 },
                { href: "/missions", label: "収束案件", icon: "triangle", level: 4 },
                { href: "/classified", label: "機密情報", icon: "classified", level: 5 },
                ...
                <Link key={item.href} href={item.href}
                  ...
                  <span className="font-mono" style={{ fontSize: "0.65rem", color: "var(--primary)" }}><DashNavIcon icon={item.icon} /></span>
                  {item.label}
                </Link>
              ))}
            </div>
    ...
    {/* [W-001] 秘密の質問未設定警告は170行目のバナーで統一表示。重複バナーを削除 */}
    </>
  );
}
