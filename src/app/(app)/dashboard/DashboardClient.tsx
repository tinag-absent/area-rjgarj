...
function GlitchText({ text, intensity }: { text: string; intensity: number }) {
...
export default function DashboardClient({
  ...
  const nextLevelXp = LEVEL_THRESHOLDS[Math.min(level + 1, 5)] ?? 2500;
  ...
  async function handleDailyLogin() {
    ...
    if (data.success) {
      ...
      updateXp(data.user.xp, data.user.level);
      addToast({
        type: "login",
        title: "デイリーログインボーナス",
