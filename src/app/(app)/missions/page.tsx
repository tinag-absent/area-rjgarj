...
function TimelineIcon({ type, color }: { type: string; color: string }) {
  const p = { width:12, height:12, fill:"none" as const, stroke:color, strokeWidth:"2" as const, strokeLinecap:"round" as const, strokeLinejoin:"round" as const };
  if (type === "alert" || type === "critical") return <svg {...p} viewBox="0 0 24 24"><path d="M10.29 3.86L1.82 18a2 2 0 0 0 1.71 3h16.94a2 2 0 0 0 1.71-3L13.71 3.86a2 2 0 0 0-3.42 0z" /><line x1="12" y1="9" x2="12" y2="13" /><line x1="12" y1="17" x2="12.01" y2="17" /></svg>;
  if (type === "deployment") return <svg {...p} viewBox="0 0 24 24"><line x1="5" y1="12" x2="19" y2="12" /><polyline points="12 5 19 12 12 19" /></svg>;
  if (type === "discovery")  return <svg {...p} viewBox="0 0 24 24"><circle cx="12" cy="12" r="10" /><circle cx="12" cy="12" r="6" /><circle cx="12" cy="12" r="2" /></svg>;
  if (type === "action")     return <svg {...p} viewBox="0 0 24 24"><polygon points="5 3 19 12 5 21 5 3" /></svg>;
  if (type === "result")     return <svg {...p} viewBox="0 0 24 24"><polyline points="20 6 9 17 4 12" /></svg>;
  return <svg {...p} viewBox="0 0 24 24"><circle cx="12" cy="12" r="4" /></svg>;
}

const TIMELINE_STYLES: Record<string, { color: string }> = {
  alert:      { color: "#ef4444" },
  deployment: { color: "#3b82f6" },
  discovery:  { color: "#a855f7" },
  action:     { color: "#eab308" },
                      ...
                      {mission.timeline.map((entry, i) => {
                        ...
                        return (
                          <div key={i} style={{ position: "relative", marginBottom: "0.5rem", display: "flex", gap: "0.5rem", alignItems: "flex-start" }}>
                            ...
                            }}><TimelineIcon type={entry.type} color={ts.color} /></div>
                            <div>
