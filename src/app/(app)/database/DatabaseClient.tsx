...
const TYPE_STYLES: Record<string, { label: string; color: string; icon: string }> = {
  "headquarters":          { label: "総本部",         color: "#00ffff", icon: "hexagon" },
  "dimensional-gate":      { label: "次元ゲート",     color: "#a78bfa", icon: "crosscircle" },
  "monitoring-station":    { label: "監視拠点",       color: "#10b981", icon: "diamond" },
  "research-facility":     { label: "研究施設",       color: "#eab308", icon: "hexagonfill" },
  "branch-office":         { label: "支局",           color: "#60a5fa", icon: "grid" },
  "field-base":            { label: "現場拠点",       color: "#f97316", icon: "triangle" },
  "containment":           { label: "封鎖区域",       color: "#ef4444", icon: "circleline" },
  "containment-facility":  { label: "収容施設",       color: "#ef4444", icon: "circleline" },
  "residue-site":          { label: "残滓採集地",     color: "#c084fc", icon: "circletarget" },
  "hazard-zone":           { label: "危険地帯",       color: "#ef4444", icon: "warning" },
  "medical-facility":      { label: "医療施設",       color: "#10b981", icon: "cross" },
  "historical-site":       { label: "歴史的遺跡",     color: "#d97706", icon: "diamondfill" },
};
...
// ── タブ定義 ──────────────────────────────────────────────────────
const TABS = [
  { id: "entities",   label: "実体カタログ",   icon: "target",   clearance: "LV2", color: "#8b5cf6" },
  { id: "modules",    label: "モジュール",     icon: "hexagon",  clearance: "LV2", color: "#3b82f6" },
  { id: "locations",  label: "ロケーション",   icon: "location", clearance: "LV1", color: "#10b981" },
  { id: "codex",      label: "コーデックス",   icon: "diamond",  clearance: "LV1", color: "#00ffff" },
  { id: "protocols",  label: "プロトコル",     icon: "chevron",  clearance: "公開", color: "#f59e0b" },
  { id: "personnel",  label: "人員ファイル",   icon: "diamond",  clearance: "LV5", color: "#f97316" },
  { id: "statistics", label: "統計",           icon: "grid",     clearance: "LV2", color: "#a855f7" },
] as const;
...
function DbIcon({ icon, color, size = 14 }: { icon: string; color: string; size?: number }) {
  ...
  if (icon === "crosscircle")  return <svg {...p} viewBox="0 0 24 24"><circle cx="12" cy="12" r="10" /><line x1="4.93" y1="4.93" x2="19.07" y2="19.07" /><line x1="19.07" y1="4.93" x2="4.93" y2="19.07" /></svg>;
  if (icon === "diamond")      return <svg {...p} viewBox="0 0 24 24"><path d="M12 2L2 9l10 13L22 9z" /><line x1="2" y1="9" x2="22" y2="9" /></svg>;
  if (icon === "hexagonfill")  return <svg {...p} fill={color} stroke="none" viewBox="0 0 24 24"><path d="M21 16V8a2 2 0 0 0-1-1.73l-7-4a2 2 0 0 0-2 0l-7 4A2 2 0 0 0 3 8v8a2 2 0 0 0 1 1.73l7 4a2 2 0 0 0 2 0l7-4A2 2 0 0 0 21 16z" /></svg>;
  ...
  if (icon === "circleline")   return <svg {...p} viewBox="0 0 24 24"><circle cx="12" cy="12" r="10" /><line x1="4.93" y1="19.07" x2="19.07" y2="4.93" /></svg>;
  ...
  if (icon === "cross")        return <svg {...p} strokeWidth="2" viewBox="0 0 24 24"><line x1="12" y1="5" x2="12" y2="19" /><line x1="5" y1="12" x2="19" y2="12" /></svg>;
  ...
  if (icon === "location")     return <svg {...p} viewBox="0 0 24 24"><path d="M21 10c0 7-9 13-9 13s-9-6-9-13a9 9 0 0 1 18 0z" /><circle cx="12" cy="10" r="3" /></svg>;
  if (icon === "chevron")      return <svg {...p} viewBox="0 0 24 24"><polyline points="9 18 15 12 9 6" /></svg>;
      ...
      <div>
        ...
        {Array.from(regionMap.entries()).map(([region, locs]) => (
          <div key={region} style={{ marginBottom: "2.5rem" }}>
            ...
            <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fill, minmax(340px, 1fr))", gap: "1rem" }}>
              {locs.map(loc => {
                const ts = TYPE_STYLES[loc.type] ?? { label: loc.type, color: "var(--muted-foreground)", icon: "circle" };
                ...
                return (
                  <div key={loc.id} className="card" style={{ borderColor: `${ts.color}20` }}>
                    <div style={{ padding: "1.25rem" }}>
                      <div style={{ display: "flex", justifyContent: "space-between", alignItems: "flex-start", marginBottom: "0.75rem" }}>
                        <span className="font-mono" style={{ fontSize: "0.7rem", color: ts.color }}><DbIcon icon={ts.icon} color={ts.color} /> {ts.label}</span>
      ...
      <div style={{ display: "flex", gap: 0, borderBottom: "1px solid rgba(255,255,255,0.08)", marginBottom: "2rem", overflowX: "auto" }}>
        {TABS.map(tab => {
          ...
          return (
            ...
            >
              <span><DbIcon icon={tab.icon} color={tab.color} /></span>
              <span>{tab.label}</span>
