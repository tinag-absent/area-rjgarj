...
const CATEGORY_META: Record<BrowseEntry["category"], { label: string; color: string; icon: string }> = {
  page:    { label: "ページ",       color: S.cyan,    icon: "diamond" },
  mission: { label: "ミッション",   color: S.yellow,  icon: "diamondfill" },
  codex:   { label: "コーデックス", color: S.purple,  icon: "target" },
  map:     { label: "マップ",       color: S.green,   icon: "location" },
  novel:   { label: "読本",         color: S.orange,  icon: "database" },
  chat:    { label: "チャット",     color: "#4fc3f7", icon: "chat" },
  other:   { label: "その他",       color: S.text3,   icon: "circle" },
};
...
function EmptyState({ icon, msg, color = "var(--muted-foreground)" }: { icon: string; msg: string; color?: string }) {
  return (
            ...
            <div key={date}>
              ...
              {(entries as BrowseEntry[]).map((entry) => {
                ...
                return (
                  ...
                  >
                    <span style={{ fontFamily: S.mono, fontSize: 16, color: meta.color, flexShrink: 0, lineHeight: 1 }}><HistIcon icon={meta.icon} color={meta.color} /></span>
...
function HistIcon({ icon, color }: { icon: string; color: string }) {
  ...
  if (icon === "database")    return <svg {...p} viewBox="0 0 24 24"><ellipse cx="12" cy="5" rx="9" ry="3" /><path d="M21 12c0 1.66-4 3-9 3s-9-1.34-9-3" /><path d="M3 5v14c0 1.66 4 3 9 3s9-1.34 9-3V5" /></svg>;
  if (icon === "chat")        return <svg {...p} viewBox="0 0 24 24"><path d="M21 15a2 2 0 0 1-2 2H7l-4 4V5a2 2 0 0 1 2-2h14a2 2 0 0 1 2 2z" /></svg>;
