...
const NPC_COLORS: Record<string, { border: string; bg: string; glow: string; name: string; dot: string }> = {
...
const NPC_USERNAMES = new Set(NPC_MEMBERS.map(n => n.username));
...
function NpcAvatar({ username, size = 32 }: { username: string; size?: number }) {
  const c = NPC_COLORS[username];
  const icon = NPC_ICONS[username] ?? "◈";
  return (
    <div style={{
      ...
      fontSize: size * 0.4, color: c?.name ?? "#fff",
      flexShrink: 0,
      boxShadow: `0 0 ${size * 0.3}px ${c?.glow ?? "transparent"}`,
    }}>
      {icon}
    </div>
  );
    ...
    <div style={{
      ...
      <div style={{ flex: 1, overflowY: "auto", padding: "0.5rem 0" }}>
        ...
        {NPC_MEMBERS.map(npc => {
          ...
          const c = NPC_COLORS[npc.username];
          return (
            <div key={npc.username} style={{
              padding: "0.5rem 0.75rem", display: "flex", alignItems: "center", gap: "0.6rem",
              transition: "background 0.2s",
              backgroundColor: isTyping ? `${c?.glow ?? "transparent"}` : "transparent",
            }}>
              <NpcAvatar username={npc.username} size={28} />
              <div style={{ flex: 1, minWidth: 0 }}>
                <div style={{ fontSize: "0.7rem", color: c?.name ?? "#fff", fontWeight: 600, fontFamily: "JetBrains Mono, monospace" }}>
                  {npc.username}
                </div>
...
function MessageBubble({ msg, agentId }: { msg: ChatMessage; agentId: string }) {
  ...
  const c = isNpc ? NPC_COLORS[msg.senderName] : null;
  const icon = isNpc ? NPC_ICONS[msg.senderName] : null;
    ...
    try {
      ...
      if (res.ok) {
        ...
        fetch("/api/npc/process", {
          method: "POST", headers: { "Content-Type": "application/json" },
          body: JSON.stringify({ chatId: CHAT_ID, messageText: sentText, senderUsername: agentId }),
        }).then(r => r.json()).then(data => {
          if (!data.responded) return;
          // 先頭NPCのタイピングインジケーター
          showTyping(data.npc, data.delayMs);
          // maxDelay後にmutate
          setTimeout(() => { mutate(); }, (data.maxDelayMs ?? data.delayMs) + 800);
          ...
          <div style={{ marginLeft: "auto", display: "flex", gap: "0.35rem" }}>
            {NPC_MEMBERS.map(npc => (
              <div key={npc.username} title={npc.username} style={{
                ...
                backgroundColor: NPC_COLORS[npc.username]?.bg,
                border: `1.5px solid ${NPC_COLORS[npc.username]?.border}`,
                display: "flex", alignItems: "center", justifyContent: "center",
                fontSize: "0.55rem", color: NPC_COLORS[npc.username]?.name,
              }}>
                {NPC_ICONS[npc.username]}
              </div>
            ))}
          </div>
        </div>
