...
function ChatChannelIcon({ icon }: { icon: string }) {
  const p = { width:13, height:13, fill:"none" as const, stroke:"currentColor", strokeWidth:"1.8" as const, strokeLinecap:"round" as const, strokeLinejoin:"round" as const };
...
export default function ChatWindow({ agentId }: { agentId: string }) {
  ...
  const channels: { id: string; label: string; icon: string }[] = [
    ...
    { id: "npc_group", label: "NPCグループ",   icon: "hexagon" },
  ];
  if (division && DIVISION_LABELS[division]) {
    channels.push({ id: `division_${division}`, label: DIVISION_LABELS[division], icon: "layers" });
  }
  ...
  // [FIX #167] error を取得してチャット履歴の読み込み失敗をUIに反映する
  const { data: messages = [], mutate, error: chatLoadError } = useSWR<ChatMessage[]>(
  ...
  useEffect(() => {
    ...
    if (messages.length > 0) {
      ...
      if (lastId) {
        // [K-001] parseInt が NaN になる文字列 ID は送信しない
    ...
    <div style={{ padding: "3rem 1.5rem", display: "flex", flexDirection: "column", height: "calc(100vh - 0px)" }}>
      ...
      <div style={{ display: "flex", gap: "0.5rem", marginBottom: "0.75rem" }}>
        {channels.map(ch => (
          <button key={ch.id} onClick={() => setActiveChannel(ch.id)}
            ...
            <ChatChannelIcon icon={ch.icon} /> {ch.label}
          </button>
        ))}
        {!division && (
      ...
      <div className="card" style={{ flex: 1, display: "flex", flexDirection: "column", minHeight: 0, borderColor: activeChannel === "npc_group" ? "rgba(80,220,120,0.2)" : "rgba(0,255,255,0.2)", position: "relative" }}>
        ...
        <div style={{ flex: 1, overflowY: "auto", padding: "1rem", display: "flex", flexDirection: "column", gap: "0.75rem" }}>
          {/* [FIX #167] チャット履歴の読み込みエラー表示 */}
          {chatLoadError && (
            <div className="font-mono" style={{ textAlign: "center", color: "#ef4444", fontSize: "0.8rem", padding: "1rem", border: "1px solid rgba(239,68,68,0.3)", background: "rgba(239,68,68,0.05)" }}>
              [通信エラー] チャット履歴の取得に失敗しました。再試行しています...
            </div>
          )}
          {messages.length === 0 && !chatLoadError && (
