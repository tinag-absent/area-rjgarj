...
export default function SkillTreeClient() {
  ...
  useEffect(() => {
    ...
    try {
      ...
      if (!res.ok) return;
      const stRes  = await apiFetch("/api/users/me/story-state");
      if (stRes.ok) {
  ...
  async function unlockSkill(skill: Skill) {
    ...
    try {
      // フラグを保存
      const flagRes = await apiFetch("/api/users/me/flags", {
        method: "POST",
      ...
      // [O-001] XP消費: activity "skill_unlock" をサーバーに送る（amount は無視されるため使わない）
      // [O-002] addXp には totalXp ではなく xpGained (delta) を渡す
      if (skill.xp > 0) {
        const xpRes = await apiFetch("/api/users/me/xp", {
          method: "POST",
          headers: { "Content-Type": "application/json" },
          body: JSON.stringify({ activity: "skill_unlock" }),
        });
        if (xpRes.ok) {
          const xpData = await xpRes.json();
          // delta（xpGained）をストアに加算する
          if (typeof xpData.xpGained === "number") addXp(xpData.xpGained);
        }
      }
