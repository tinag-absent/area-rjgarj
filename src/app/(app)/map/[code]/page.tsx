...
export default async function MunicipalityDetailPage(
  ...
  // [FIX #202] idx=0 のとき idx-1 = -1 → array[-1] は undefined になるため明示的に bounds チェックする
  const idx      = MUNICIPALITIES.findIndex(m => m.code === code);
  const neighbors = [
    idx > 0 ? MUNICIPALITIES[idx - 1] : undefined,
    idx < MUNICIPALITIES.length - 1 ? MUNICIPALITIES[idx + 1] : undefined,
  ].filter((m): m is typeof MUNICIPALITIES[number] => m !== undefined);
