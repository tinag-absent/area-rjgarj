...
export default function LoginForm() {
  ...
  useEffect(() => {
    try {
      ...
      if (saved) {
        const { agentId, password } = JSON.parse(saved);
        if (agentId) setLoginId(agentId);
        if (password) setLoginPw(password);
        setRememberMe(true);
      }
    } catch { /* 無視 */ }
  }, []);
  ...
  async function handleLogin(e: React.FormEvent) {
    ...
    try {
      ...
      if (rememberMe) {
        localStorage.setItem(STORAGE_KEY, JSON.stringify({ agentId: loginId, password: loginPw }));
      } else {
