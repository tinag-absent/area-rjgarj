  ...
  return (
    <div>
      <div style={{ display: "flex", justifyContent: "space-between", marginBottom: 6, fontFamily: S.mono, fontSize: 10 }}>
        ...
        <span style={{ color: S.text3 }}>{xp % XP_PER_LEVEL} / {XP_PER_LEVEL} XP</span>
      </div>
      ...
      <div style={{ marginTop: 4, fontFamily: S.mono, fontSize: 9, color: S.text3, textAlign: "right" }}>
        次のレベルまで {XP_PER_LEVEL - (xp % XP_PER_LEVEL)} XP
      </div>
    </div>
  );
}
...
export default function ProfilePage() {
  ...
  const [col, bg] = LEVEL_COLORS[(displayUser.level as number) ?? 0] ?? LEVEL_COLORS[0];
  const initials = ((displayUser.name as string) || "?").charAt(0);

  const radarStats = [
    { label: "XP", value: Math.min(100, ((displayUser.xp as number) / 5000) * 100) },
    { label: "ログイン", value: Math.min(100, ((displayUser.loginCount as number) / 100) * 100) },
    { label: "活動量", value: Math.min(100, (browseHistory.length / 50) * 100) },
    { label: "探索", value: Math.min(100, (searchHistory.length / 30) * 100) },
    { label: "継続", value: Math.min(100, (((displayUser.streak as number) ?? 0) / 30) * 100) },
    { label: "安定度", value: Math.max(0, 100 - ((displayUser.anomalyScore as number) ?? 0)) },
  ];
    ...
    <div style={{ minHeight: "calc(100vh - 4rem)", margin: "-2rem -1.5rem", background: S.bg, overflowY: "auto" }}>
      ...
      <div style={{
        ...
        <div style={{
          ...
          <div style={{
            ...
            background: (displayUser.status as string) === "active" ? S.green : S.yellow,
        ...
        <div style={{ flex: 1 }}>
          <div style={{ fontSize: 22, fontWeight: 500, color: S.text, marginBottom: 4 }}>
            {displayUser.name as string}
          </div>
          <div style={{ fontFamily: S.mono, fontSize: 12, color: S.text3, marginBottom: 10 }}>
            {displayUser.agentId as string}
          </div>
          <div style={{ display: "flex", gap: 8, flexWrap: "wrap" }}>
            <Badge text={`LV${displayUser.level} — ${LEVEL_TITLES[(displayUser.level as number)] ?? "不明"}`} color={col} />
            <Badge text={(displayUser.role as string).toUpperCase()} color={(displayUser.role as string) === "admin" ? S.red : S.text3} />
            <Badge text={(displayUser.divisionName as string) ?? "未配属"} color={S.purple} />
            <Badge text={(displayUser.status as string).toUpperCase()} color={(displayUser.status as string) === "active" ? S.green : S.yellow} />
          </div>
        </div>
        ...
        <div style={{ fontFamily: S.mono, fontSize: 10, color: S.text3, textAlign: "right", flexShrink: 0 }}>
          ...
          <div style={{ color: S.text2, marginTop: 2 }}>
            {(displayUser.createdAt as string) ? new Date(displayUser.createdAt as string).toLocaleDateString("ja-JP") : "—"}
          </div>
          <div style={{ marginTop: 8 }}>最終ログイン</div>
          <div style={{ color: S.text2, marginTop: 2 }}>
            {(displayUser.lastLogin as string) ? new Date(displayUser.lastLogin as string).toLocaleDateString("ja-JP", { month: "short", day: "numeric", hour: "2-digit", minute: "2-digit" }) : "—"}
          </div>
        </div>
      </div>
      ...
      <div style={{ padding: "24px 32px", display: "flex", flexDirection: "column", gap: 24 }}>
        ...
        <div style={{ background: S.panel, border: `1px solid ${S.border}`, padding: "18px 20px" }}>
          ...
          <XpBar xp={displayUser.xp as number} level={displayUser.level as number} />
        </div>
        ...
        <div>
          ...
          <div style={{ display: "flex", gap: 10, flexWrap: "wrap" }}>
            <StatBox value={displayUser.xp as number} label="累計XP" color={col} />
            <StatBox value={displayUser.loginCount as number} label="ログイン回数" color={S.cyan} />
            <StatBox value={(displayUser.streak as number) ?? 0} label="連続ログイン日" color={S.yellow} />
            <StatBox value={browseHistory.length} label="閲覧記録" color={S.purple} />
            <StatBox value={searchHistory.length} label="検索記録" color={S.orange} />
            <StatBox value={(displayUser.anomalyScore as number) ?? 0} label="異常スコア" color={(displayUser.anomalyScore as number) > 30 ? S.red : S.green} />
          </div>
        </div>
        ...
        <div style={{ display: "flex", gap: 16 }}>
          ...
          <div style={{ background: S.panel, border: `1px solid ${S.border}`, padding: "18px 20px", flex: 1 }}>
            ...
            <ActivityBar label="XP" value={(displayUser.xp as number)} max={5000} color={col} />
            <ActivityBar label="ログイン" value={(displayUser.loginCount as number)} max={100} color={S.cyan} />
            <ActivityBar label="連続日" value={(displayUser.streak as number) ?? 0} max={30} color={S.yellow} />
            ...
            <ActivityBar label="安定度" value={Math.max(0, 100 - ((displayUser.anomalyScore as number) ?? 0))} max={100} color={S.green} />
            ...
            <div style={{ marginTop: 20, padding: "12px 14px", background: S.panel2, border: `1px solid ${S.border2}` }}>
              ...
              <div style={{ display: "flex", alignItems: "center", gap: 12 }}>
                <div style={{ flex: 1, height: 8, background: S.border2 }}>
                  <div style={{
                    height: "100%",
                    width: `${(displayUser.observerLoad as number) ?? 0}%`,
                    background: (displayUser.observerLoad as number) > 70 ? S.red : (displayUser.observerLoad as number) > 40 ? S.yellow : S.green,
                    transition: "width .8s ease",
                  }} />
                </div>
                <span style={{ fontFamily: S.mono, fontSize: 13, color: S.text, minWidth: 40 }}>
                  {(displayUser.observerLoad as number) ?? 0}%
                </span>
              </div>
              <div style={{ fontFamily: S.mono, fontSize: 9, color: S.text3, marginTop: 6 }}>
                {(displayUser.observerLoad as number) > 70 ? "⚠ 観測者負荷が高い状態です" : "正常範囲内"}
              </div>
            </div>
          </div>
        </div>
