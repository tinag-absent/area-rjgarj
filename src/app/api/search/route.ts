...
// ── JSONローダー（モジュールレベルキャッシュ: バグ#18修正）────────────────
// readFileSyncをリクエストごとに実行するとNode.jsイベントループをブロックするため、
// サーバー起動時に一度だけ読み込んでメモリにキャッシュする。

const _jsonCache: Map<string, unknown> = new Map();

function loadJson<T>(file: string): T {
  if (_jsonCache.has(file)) return _jsonCache.get(file) as T;
...
function searchMissions(q: string, level: number): SearchResult[] {
  ...
  return missions
    .filter(m => (m.securityLevel ?? 0) <= level) // [Q-012] クリアランスレベルでフィルタ
...
function searchLocations(q: string, level: number): SearchResult[] {
  const { locations } = loadJson<{ locations: Array<{
    id: string; name: string; type: string; description: string; securityLevel: number;
  }> }>("locations-data.json");

  return locations
    .filter(l => (l.securityLevel ?? 0) <= level) // [BUG-F FIX] クリアランスレベルでフィルタ（missionsと同様）
...
export async function GET(req: NextRequest) {
  ...
  // [FIX-C10] 検索クエリ長を制限（DoS防止）
  if (q.length > 200) {
    return NextResponse.json({ error: "検索クエリが長すぎます" }, { status: 400 });
  }
  ...
  try {
    ...
    if (category === "all" || category === "mission")   results.push(...searchMissions(q, level));
    ...
    if (category === "all" || category === "location")  results.push(...searchLocations(q, level));
