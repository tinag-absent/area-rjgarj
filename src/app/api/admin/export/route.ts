...
export async function GET(req: NextRequest) {
  // [AB-011] ユーザーデータの一括エクスポートは super_admin のみ
  const auth = requireAdmin(req);
  if (auth instanceof NextResponse) return auth;
  if (auth.user.role !== "super_admin") {
    return NextResponse.json({ error: "データエクスポートには super_admin 権限が必要です" }, { status: 403 });
  }
  ...
  try {
    ...
    // [FIX-H14] エクスポートに上限を設けてメモリ枯渇・タイムアウトを防止
    const EXPORT_LIMIT = 50000;
    const offsetParam = parseInt(new URL(req.url).searchParams.get("offset") ?? "0", 10);
      ...
      case "xp_logs":
        rows = await query<Record<string, unknown>>(db,
          `SELECT xl.user_id, u.username, xl.activity, xl.xp_gained, xl.created_at
           ...
           ORDER BY xl.created_at DESC LIMIT ? OFFSET ?`,
          [EXPORT_LIMIT, safeOffset]
        );
      ...
      case "flags":
        rows = await query<Record<string, unknown>>(db,
          `SELECT pf.user_id, u.username, pf.flag_key, pf.flag_value, pf.set_at
           ...
           ORDER BY pf.set_at DESC LIMIT ? OFFSET ?`,
          [EXPORT_LIMIT, safeOffset]
        );
        filename = "flags.csv";
        break;
      ...
      case "anomaly":
        rows = await query<Record<string, unknown>>(db,
          `SELECT u.id, u.username, u.anomaly_score, u.status,
           ...
           GROUP BY u.id ORDER BY u.anomaly_score DESC LIMIT ? OFFSET ?`,
          [EXPORT_LIMIT, safeOffset]
        ).catch(async () =>
          // anomaly_logs が未作成の場合のフォールバック
          query<Record<string, unknown>>(db,
            `SELECT id, username, anomaly_score, status FROM users
             WHERE deleted_at IS NULL ORDER BY anomaly_score DESC LIMIT ? OFFSET ?`,
            [EXPORT_LIMIT, safeOffset]
          )
        );
