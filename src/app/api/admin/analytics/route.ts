  ...
  try {
    const [
      ...
      query<{ username: string; xp: number; level: number }>(db, `
        SELECT u.username, u.xp_total AS xp, u.clearance_level AS level
        FROM users u
        WHERE u.deleted_at IS NULL
        ORDER BY u.xp_total DESC LIMIT 10
      `),
      ...
      query<{ chat_id: string; msg_count: number }>(db, `
        ...
        FROM chat_messages GROUP BY chat_id ORDER BY msg_count DESC LIMIT 10
      `),
