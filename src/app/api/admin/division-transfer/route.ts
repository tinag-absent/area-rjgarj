...
export async function GET(req: NextRequest) {
  // [T-003] requireSuperAdminからrequireAdminへ変更（adminロールも閲覧できるよう）
...
// POST: 申請を新規作成
// [FIX-H03] /api/admin/ 配下のエンドポイントであるため requireAdmin を使用する。
// 一般プレイヤーの部門移動申請は /api/users/me/division-transfer エンドポイントで受け付ける想定。
// 管理者が特定ユーザーの代わりに申請を作成できる管理機能として位置づける。
export async function POST(req: NextRequest) {
  const auth = requireAdmin(req);
  if (auth instanceof NextResponse) return auth;
  const { user: authUser } = auth;

  const { toDivisionId, reason, targetUserId } = await req.json().catch(() => ({}));
  if (!toDivisionId) return NextResponse.json({ error: "移動先部門が必要です" }, { status: 400 });

  // [FIX-H03] 管理者が対象ユーザーIDを指定できる（省略時は管理者自身）
  const userId = typeof targetUserId === "string" && targetUserId.trim() ? targetUserId.trim() : authUser.userId;

  const db = getDb();

  try {
    // [AE-023] toDivisionId の存在確認
    const divCheck = await db.execute({
      sql: `SELECT id FROM divisions WHERE id = ? AND is_active = 1 LIMIT 1`,
      args: [toDivisionId],
    });
    if (divCheck.rows.length === 0) {
      return NextResponse.json({ error: "指定された移動先部門が存在しません" }, { status: 400 });
    }
...
export async function PUT(req: NextRequest) {
  // [T-003] requireSuperAdminからrequireAdminへ変更
  const auth = requireAdmin(req);
  if (auth instanceof NextResponse) return auth;
