...
export async function POST(req: NextRequest) {
  ...
  // [FIX] id フォーマット検証
  if (!id || typeof id !== "string" || !/^[\w\-]{1,64}$/.test(id)) {
    return NextResponse.json({ error: "id は64文字以内の英数字・ハイフン・アンダースコアにしてください" }, { status: 400 });
  }
  // [FIX] data_json サイズ上限（200KB）
  const dataJson = JSON.stringify({ id, ...rest });
  if (dataJson.length > 200_000) {
    return NextResponse.json({ error: "スキルツリーデータが大きすぎます（200KB以内）" }, { status: 400 });
  }
  // [FIX] sort_order は整数であること
  const sortOrder = typeof sort_order === "number" && Number.isInteger(sort_order) ? sort_order : 0;
  ...
  await execute(db,
    ...
    [id, active?1:0, sortOrder, dataJson]
  );
