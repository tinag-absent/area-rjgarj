...
export async function POST(req: NextRequest) {
  ...
  // [FIX-NEW] id フォーマット検証
  if (!id || typeof id !== "string" || !/^[\w\-]{1,128}$/.test(id)) {
    return NextResponse.json({ error: "id は128文字以内の英数字・ハイフン・アンダースコアにしてください" }, { status: 400 });
  }
  // [FIX-NEW] conditionType のホワイトリスト検証
  const ALLOWED_CONDITION_TYPES = ["flag", "level", "xp", "loginCount", "streak"];
  if (!ALLOWED_CONDITION_TYPES.includes(conditionType)) {
    return NextResponse.json({ error: `conditionType は ${ALLOWED_CONDITION_TYPES.join(" / ")} のいずれかです` }, { status: 400 });
  }
  // [FIX-NEW] conditionMin は整数であることを検証
  const minVal = typeof conditionMin === "number" ? Math.floor(conditionMin) : 0;
  if (minVal < 0) {
    return NextResponse.json({ error: "conditionMin は0以上の整数にしてください" }, { status: 400 });
  }
  ...
  await execute(db,
    ...
    [id, active?1:0, secret?1:0, icon||'🏆', color||'#ffd740', name||'', desc||'', conditionType||'flag', conditionKey||'', conditionValue||'', minVal]
  );
