...
export async function POST(req: NextRequest) {
  ...
  // [BUG-C FIX] suspended/inactive ユーザーもパスワードリセット後にJWTを発行しない。
  // パスワードは変更できるが、ログイン状態にはしない。
  if (user.status === "suspended") {
    return NextResponse.json({ error: "このアカウントは一時停止中です。管理者にお問い合わせください。" }, { status: 403 });
  }
  ...
  // [U-001] suspended/inactiveユーザーをactiveに昇格させない。bannedはここに到達しない。
  // statusはそのまま維持し、パスワードのみ変更する
...
export async function PATCH(req: NextRequest) {
  ...
  // [FIX] 秘密の質問の長さ制限
  if (typeof newQuestion !== "string" || newQuestion.trim().length < 4 || newQuestion.length > 200) {
    return NextResponse.json({ error: "秘密の質問は4〜200文字にしてください" }, { status: 400 });
  }
  if (newAnswer.trim().length < 1) {
    return NextResponse.json({ error: "回答を入力してください" }, { status: 400 });
  }
  if (newAnswer.trim().length > 200) {
    return NextResponse.json({ error: "回答は200文字以内にしてください" }, { status: 400 });
  }
