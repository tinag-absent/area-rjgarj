...
export async function POST(request: NextRequest) {
  ...
  const { xp: xpReward, rateLimit } = await resolveXp(activity, user.id, user.level, db);
  ...
  await execute(db, "UPDATE users SET xp=?, level=? WHERE id=?", [newXp, newLevel, user.id]);
  ...
  return Response.json({ xpGained: xpReward, leveledUp, newLevel, totalXp: newXp });
}
