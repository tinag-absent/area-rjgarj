...
export interface Trigger {
  ...
  /**
   * effects をユーザー情報から動的に生成する場合はこちらを使用。
   * 定義されている場合は effects より優先して使用される。
   */
...
/**
 * Trigger から実際に適用する TriggerEffect を取得する。
 * getEffects が定義されている場合はそちらを優先する。
 */
export function resolveEffects(trigger: Trigger, user: TriggerUser): TriggerEffect {
  return trigger.getEffects ? trigger.getEffects(user) : trigger.effects;
}
...
export const TRIGGERS: Trigger[] = [
  ...
  },
  {
    ...
    effects: {
      ...
      notification: { type: "warning", title: "異常スコア警告", body: "あなたの行動パターンに異常が検出されました。" },
    },
    getEffects: (u) => ({
      flag: "anomaly_detected",
      notification: { type: "warning", title: "異常スコア警告", body: `あなたの行動パターンに異常が検出されました。スコア: ${u.anomalyScore}` },
    }),
  },
  {
    id: "observer_warned",
