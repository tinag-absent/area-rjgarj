...
// npc-config.ts の NPC_USERNAMES（クライアント用）と内容は同一だが、
// こちらはサーバーサイドの NPC エンジンが NPCS マップから直接生成する正規版。
...
export function pickRandom<T>(arr: T[]): T {
  if (arr.length === 0) throw new Error("[npc-engine] pickRandom: 空配列には使用できません");
...
/**
 * NPC返答を生成する
 * @returns { npc, text } or null（沈黙の場合）
 */
