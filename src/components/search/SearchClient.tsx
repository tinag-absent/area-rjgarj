...
import { useDiscoveredStore } from "@/store/discoveredStore";
...
const TABS = [
  { id: "all",       label: "すべて",       color: "#00ffff", icon: "diamond" },
  { id: "mission",   label: "収束案件",     color: "#ef4444", icon: "bolt" },
  { id: "entity",    label: "海蝕実体",     color: "#a855f7", icon: "target" },
  { id: "module",    label: "モジュール",   color: "#eab308", icon: "hexagon" },
  { id: "location",  label: "ロケーション", color: "#10b981", icon: "location" },
  { id: "personnel", label: "人員",         color: "#f97316", icon: "diamond" },
  { id: "novel",     label: "ノベル",       color: "#06b6d4", icon: "diamondfill" },
  { id: "post",      label: "掲示板",       color: "#64748b", icon: "chevron" },
] as const;
...
function ResultCard({
  ...
  result: SearchResult;
  onDiscover: (r: SearchResult) => void;
      ...
      >
        ...
        {isDiscovered && (
          <div style={{
            ...
            color: "rgba(0,255,255,0.4)", letterSpacing: "0.06em",
          }}>
            ✓ LOGGED
          </div>
        )}
...
function SearchCatIcon({ icon, color }: { icon: string; color: string }) {
  const p = { width:14, height:14, fill:"none" as const, stroke:color, strokeWidth:"1.8" as const, strokeLinecap:"round" as const, strokeLinejoin:"round" as const };
  if (icon === "diamond")     return <svg {...p} viewBox="0 0 24 24"><path d="M12 2L2 9l10 13L22 9z" /><line x1="2" y1="9" x2="22" y2="9" /></svg>;
  if (icon === "bolt")        return <svg {...p} viewBox="0 0 24 24"><polygon points="13 2 3 14 12 14 11 22 21 10 12 10 13 2" /></svg>;
...
export default function SearchClient() {
  ...
  const addItem = useDiscoveredStore((s) => s.addItem);
  const hasItem = useDiscoveredStore((s) => s.hasItem);
  ...
  const doSearch = useCallback(async (q: string) => {
    ...
    setError("");
    try {
      // [AE-018] raw fetch → apiFetch に変更（CSRF保護ヘッダー付与）
      const res = await apiFetch(`/api/search?q=${encodeURIComponent(trimmed)}&category=all`);
  ...
  // 結果カードがクリックされたときに発見済みとして記録
  function handleDiscover(result: SearchResult) {
    addItem({ ...result, discoveredQuery: lastQuery });
  }
            ...
            <div style={{ display: "flex", flexDirection: "column", gap: "0.375rem" }}>
              {displayResults.map(r => (
                <ResultCard
                  ...
                  isDiscovered={hasItem(r.id, r.category)}
                />
              ))}
            </div>
          )}
